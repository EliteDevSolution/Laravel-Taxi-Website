...
! function(t) {
	...
	function i(e) {
		var s, n, a, o, l, d, r = this,
			c = this.find("ul"),
			u = this.data();
		return u.vals && 0 !== c.length ? ("undefined" == typeof e && (e = u.iSec), s = u.vals[e], n = c.eq(e), a = n.children(), o = u.countdown ? -1 : 1, a.eq(1).html(s), s += o, e === u.iSec && (l = u.tickTimeout, d = t.now() - u.ttStartTime, u.sec += o, l += Math.abs(u.seconds - u.sec) * l - d, u.intervalId = setTimeout(i.bind(this), l)), s < 0 || s > u.limits[e] ? (s < 0 ? (s = u.limits[e], e === u.iHour && u.displayDays > 0 && 0 === u.vals[e - 1] && (s = 3)) : s = 0, e > 0 && i.call(this, e - 1)) : !u.countdown && e === u.iHour && u.displayDays > 0 && 2 === u.vals[e - 1] && 3 === u.vals[e] && (s = 0, i.call(this, e - 1)), a.eq(0).html(s), t.support.transition ? (n.addClass("transition"), n.css({
			top: 0
		}), setTimeout(function() {
	...
	return "undefined" == typeof t.support.transition && (t.support.transition = function() {
		...
		return g.countdown !== !1 && g.seconds > s && "undefined" == typeof g.displayDays ? (h = Math.floor(g.seconds / s), g.displayDays = h < 10 && 1 || h < 100 && 2 || 3) : g.displayDays === !0 ? g.displayDays = 3 : g.displayDays && (g.displayDays = g.displayDays > 0 ? Math.floor(g.displayDays) : 3), this.each(function() {
			var i, s, n, d, r, u, h, p, f, m, y, v, T, w, I, S, x, b, M, k, D, z, C = t(this),
				H = C.data();
			if (H.intervalId && (clearInterval(H.intervalId), H.intervalId = null), H.vals) "reset" !== c && t.extend(H, g);
			else {
				if (s = H.opt ? H.options : g, i = Object.keys(a).reduce(function(t, e) {
						return Array.isArray(a[e]) ? t[e] = a[e].slice(0) : t[e] = a[e], t
					}, {}), H = t.extend(i, s), H.options = s, H.height = Math.round(100 * H.fontSize / 93) || H.height, H.width = Math.round(.8 * H.fontSize + .13 * H.height) || H.width, H.displayHours = !(!H.displayDays && !H.displayHours), d = {
						fontFamily: H.fontFamily
					}, H.fontSize > 0 && (d.fontSize = H.fontSize + "px"), r = H.languages[H.lang] || l[H.lang], C.addClass("timeTo").addClass("timeTo-" + H.theme).css(d), u = Math.round(H.height / 10), h = '<ul style="left:' + u + "px; top:-" + H.height + 'px"><li>0</li><li>0</li></ul></div>', p = H.fontSize ? ' style="width:' + H.width + "px; height:" + H.height + 'px;"' : ' style=""', f = '<div class="first"' + p + ">" + h, m = "<div" + p + ">" + h, y = "<span>:</span>", v = Math.round(2 * H.width + 3), T = H.captionSize || H.fontSize && Math.round(.43 * H.fontSize), w = T ? "font-size:" + T + "px;" : "", I = T ? ' style="' + w + '"' : "", S = (H.displayCaptions ? (H.displayHours ? '<figure style="max-width:' + v + 'px">$1<figcaption' + I + ">" + r.hours + "</figcaption></figure>" + y : "") + '<figure style="max-width:' + v + 'px">$1<figcaption' + I + ">" + r.min + "</figcaption></figure>" + y + '<figure style="max-width:' + v + 'px">$1<figcaption' + I + ">" + r.sec + "</figcaption></figure>" : (H.displayHours ? "$1" + y : "") + "$1" + y + "$1").replace(/\$1/g, f + m), H.displayDays > 0) {
					for (x = .4 * H.fontSize || a.gap, b = f, n = H.displayDays - 1; n > 0; n -= 1) b += 1 === n ? m.replace('">', "margin-right:" + Math.round(x) + 'px">') : m;
					S = (H.displayCaptions ? '<figure style="width:' + Math.round(H.width * H.displayDays + x + 4) + 'px">$1<figcaption style="' + w + "padding-right:" + Math.round(x) + 'px">' + r.days + "</figcaption></figure>" : "$1").replace(/\$1/, b) + S
				}
				C.html(S)
			}
			if (M = C.find("div"), M.length < H.vals.length) {
				for (k = H.vals.length - M.length, D = H.vals, z = H.limits, H.vals = [], H.limits = [], n = 0; n < M.length; n += 1) H.vals[n] = D[k + n], H.limits[n] = z[k + n];
				H.iSec = H.vals.length - 1, H.iHour = H.vals.length - 5
			}
			H.sec = H.seconds, C.data(H), c && o[c] ? o[c].call(C, H.seconds) : H.start ? o.start.call(C, H.seconds) : e.call(C, H.seconds)
		})
	}, t
});