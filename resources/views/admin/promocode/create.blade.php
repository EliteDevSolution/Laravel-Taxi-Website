    	...
    	<div class="box box-block bg-white">
            ...
            <form class="form-horizontal" action="{{route('admin.promocode.store')}}" method="POST" enctype="multipart/form-data" role="form">
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="number" value="{{ old('percentage') }}" name="percentage" required id="percentage" placeholder="Percentage">
					</div>
				</div>
				<div class="form-group row">
					<label for="max_amount" class="col-xs-2 col-form-label">@lang('admin.promocode.max_amount')</label>
					<div class="col-xs-10">
						<input type="number" class="form-control" name="max_amount" required id="max_amount" placeholder="Max Amount" value="{{old('max_amount')}}">
					</div>
				</div>
