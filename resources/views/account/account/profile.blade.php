    	...
    	<div class="box box-block bg-white">
            ...
            <form class="form-horizontal" action="{{route('account.profile.update')}}" method="POST" enctype="multipart/form-data" role="form">
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="text" value="{{ Auth::guard('account')->user()->name }}" name="name" required id="name" placeholder=" @lang('admin.name')">
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="email" required name="email" value="{{ isset(Auth::guard('account')->user()->email) ? Auth::guard('account')->user()->email : '' }}" id="email" placeholder="@lang('admin.email')">
					</div>
				</div>
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="text" required name="mobile" value="{{ isset(Auth::guard('account')->user()->mobile) ? Auth::guard('account')->user()->mobile : '' }}" id="mobile" placeholder="@lang('admin.mobile')">
					</div>
				</div>
				<div class="form-group row">
